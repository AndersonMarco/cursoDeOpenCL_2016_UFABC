...
int main(void){
    ...
    catchCode=loadOpenclKernel(&kernelSrc,kernelPath); checkMallocMain(kernelSrc); checkGenericMain(catchCode); 
    ...
    catchCode = clSetKernelArg(kernel, 3, sizeof(int), (void *)&vectorsLength);checkCLMain(catchCode);
